...
from loss_util import ContrastiveLoss, Cross_entropy
from model_pt_clip import ModelCombi
import time
import os
import pdb
from torch.utils.tensorboard import SummaryWriter

keyword = "classi_contrastloss"
...
for epoch in tqdm(range(num_epochs)):
    ...
    model.eval()
    with torch.no_grad():   
        for sketches, pcds, target, pos_neg_ind in te_data_loader:
            sketches = sketches.float().to(device)
            pcds = pcds.float().to(device)
            label = target.to(device)
            pos_neg_ind = pos_neg_ind.to(device)
