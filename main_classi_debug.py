...
from data_classi_debug import ShapeData_skt, ShapeData_3d, ShapeData, pairing, read_classification_file 
from loss_util import ContrastiveLoss
from model_classi import basicmodel
import pdb
from itertools import cycle
from torch.utils.tensorboard import SummaryWriter
writer = SummaryWriter('runs/classification_sanity')
...
for epoch in tqdm(range(num_epochs)):
    ...
    for ind,(sketches, pcds, target) in enumerate(tr_data_loader):
        ...
        outputs, skt_emd, pcd_emd = model(sketches, pcds.transpose(1, 2))
        
        #plot the embeddings
        # writer.add_embedding(pcd_emd, metadata=label, global_step=ind, tag="pcd")
        # writer.add_embedding(skt_emd, metadata=label, global_step=ind+1, tag="skt")
        ...
        #         print(ind)
        #         sys.exit()
        all_pcd_embeddings.append(pcd_emd.cpu().detach().numpy())  
        all_skt_embeddings.append(skt_emd.cpu().detach().numpy())
        all_labels.append(label.cpu().detach().numpy())

    # Convert lists to numpy arrays
    all_pcd_embeddings = np.concatenate(all_pcd_embeddings, axis=0)
    all_skt_embeddings = np.concatenate(all_skt_embeddings, axis=0)
    all_labels = np.concatenate(all_labels, axis=0)

    # Apply t-SNE
    tsne = TSNE(n_components=2, random_state=42, perplexity=30)
    pcd_2d = tsne.fit_transform(all_pcd_embeddings)
    skt_2d = tsne.fit_transform(all_skt_embeddings)

    # Plot and log to TensorBoard
    fig, ax = plt.subplots(1, 2, figsize=(12, 6))

    sc1 = ax[0].scatter(pcd_2d[:, 0], pcd_2d[:, 1], c=all_labels, cmap="jet", alpha=0.7)
    ax[0].set_title("t-SNE of Point Cloud Embeddings")
    fig.colorbar(sc1, ax=ax[0])

    sc2 = ax[1].scatter(skt_2d[:, 0], skt_2d[:, 1], c=all_labels, cmap="jet", alpha=0.7)
    ax[1].set_title("t-SNE of Sketch Embeddings")
    fig.colorbar(sc2, ax=ax[1])

    writer.add_figure("t-SNE Scatter Plot", fig)
    ...
    with torch.no_grad():   
        for ind,(sketches, pcds, target) in enumerate(te_data_loader):
            ...
            # writer.add_embedding(pcd_emd, metadata=label, global_step=ind, tag="pcd_val")
            # writer.add_embedding(skt_emd, metadata=label, global_step=ind+1, tag="skt_val")
