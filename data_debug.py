...
def pairing(sketch_models, models_3d):
    ...
    print("sketch_modles: ", len(sketch_models))
    print("models_3d: ", len(models_3d))
    print()
    pairs = []
    all_classes = set(sketch_models.keys()) & set(models_3d.keys())
    for class_name in all_classes:
        #positive pairs (target = 0)
        sketch_ids = sketch_models[class_name]
        model_ids = models_3d[class_name]
        if len(model_ids) == 0:
                print(f"hn ji, no model in class {class_name}")
                continue  
        ...
        for i in sketch_ids:
            ...
            pairs.append((i, pos_ind, class_name, 0))
        ...
        for i in sketch_ids:
            ...
            if len(models_3d[neg_cls]) == 0:
                continue             
            neg_ind = random.choice(models_3d[neg_cls])
            # print("neg_ind: ", neg_ind)
            pairs.append((i, neg_ind, class_name, 1)) 
