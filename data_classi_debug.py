...
def read_classification_file(filename, flag, class_path):
    ...
    if flag == "models":
        ...
        while i < len(lines):
            ...
            # print(f"class {class_name} has {num_models} models")
            # print("model_tr: ", len(model_ids_tr))
            # print("model_te: ", len(model_ids_te))
            # if num_models == 1 or num_models == 0:
            #     lame_class += 1
            # print(f"lame classes: {lame_class}")
            # print(model_ids)
...
def pairing(sketch_models, models_3d):
    ...
    # print("sketch classes: ", len(sketch_models.keys()))
    # print("3d classes: ", len(models_3d.keys()))
    ...
    for class_name in all_classes:
        ...
        # print("sketch_ids: ", len(sketch_ids))
        # print("model_ids: ", len(model_ids))
        if len(model_ids) == 0:
                print(f"In data pairing, skipped {class_name}, ")
                continue  
...
class ShapeData_skt(Dataset):
    def __init__(self, path, data_tup, transform=None, label="Train"):
        self.path = path
        # self.data_tup, self.N = data_tup
    ...
    def __getitem__(self, index):
        data, data_class, cls_id = self.data_tup[index]
        data_path = os.path.join(self.path, f"{data_class}/{self.label}/{data}.png")
        data = self.load_image(data_path)
        if self.transform:
            data = self.transform(data)
        return (data, torch.tensor(cls_id)) 
    
class ShapeData_3d(Dataset):
    def __init__(self, path, data_tup, transform=None, label="Train"):
        self.path = path
        # self.data_tup, self.N = data_tup
        self.data_tup= data_tup
    ...
    def __getitem__(self, index):
        data, data_class, cls_id = self.data_tup[index]
        data_path = os.path.join(self.path, f"M{data}.npy")
        data = self.load_mesh(data_path)
        if len(data) == 0:
            return None, None
        if self.transform:
            data = self.transform(data) 
