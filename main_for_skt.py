...
optimizer = torch.optim.Adam(model_skt.parameters(), lr=0.0005)
...
for epoch in tqdm(range(num_epochs)):
    ...
    for ind,(sketches, pcds, target) in enumerate(tr_data_loader):
        ...
        if ind == 0:
            ...
            show_batch(sketches[:4], label[:4], save_path=f"/nlsasfs/home/neol/rushar/scripts/img_to_pcd/train_loop_plots/debug_batch_{epoch}.png")
    ...
    with torch.no_grad():   
        for ind, (sketches, pcds, target) in enumerate(te_data_loader):
            ...
            if ind == 0:
                print("labels: ", label, flush=True)
                print("outputs: ", outputs.argmax(dim=1), flush=True)
                show_batch(sketches[:4], label[:4], save_path=f"/nlsasfs/home/neol/rushar/scripts/img_to_pcd/train_loop_plots/debug_batch_val_{epoch}.png")
