...
keyword = "conti_cross_with_sampler"
...
tr_samplers = samplers.MPerClassSampler(
    tr_dataset,
    m=4,  # Number of samples per class
    batch_size=B
)

te_samplers = samplers.MPerClassSampler(
    te_dataset,
    m=4,  # Number of samples per class
    batch_size=B
)
...
tr_data_loader = DataLoader(tr_dataset, batch_size=B, num_workers=0, pin_memory=True, drop_last=True, sampler=tr_samplers)
te_data_loader = DataLoader(te_dataset, batch_size=B, num_workers=0, pin_memory=True, drop_last=True, sampler=te_samplers)
...
# for ind, i in enumerate(te_data_loader):
#     if ind == (len(te_data_loader) - 1):
#         print("te_data_loader shape: ") 
#         print(i[0].shape, i[1].shape, i[2].shape)
#         # visualize_pcd(i[1][0], "original")
#         # pdb.set_trace()
#         break   
...
model.load_state_dict(torch.load("/nlsasfs/home/neol/rushar/scripts/img_to_pcd/saved_models/cross_with_sampler/model_2.pt"))
...
for epoch in tqdm(range(num_epochs)):
    ...
    for ind,batch in enumerate(tr_data_loader):
        (sketches, pcds, target, pos_neg_ind) = batch
        sfd_ind = torch.randperm(len(sketches))
        sketches = sketches[sfd_ind].float().to(device) 
        pcds = pcds[sfd_ind].float().to(device)
        label = target[sfd_ind].to(device)
        pos_neg_ind = pos_neg_ind[sfd_ind].to(device)
    ...
    model.eval()
    with torch.no_grad():   
        for ind,batch in enumerate(te_data_loader):
            (sketches, pcds, target, pos_neg_ind) = batch
            sfd_ind = torch.randperm(len(sketches))
            sketches = sketches[sfd_ind].float().to(device) 
            pcds = pcds[sfd_ind].float().to(device)
    ...
    print("epoch time: ", time.time() - st, flush=True)
